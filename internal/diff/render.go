...
// IsColorSupported returns true if the given writer appears to be a terminal
// that supports ANSI colors. Returns false if the NO_COLOR env var is set.
func IsColorSupported(w io.Writer) bool {
	if _, ok := os.LookupEnv("NO_COLOR"); ok {
		return false
	}

	// Check if writer is an *os.File (stdout/stderr).
